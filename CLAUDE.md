...
**Rule 5: Context**
Always find ways to reduce the context window usage or context across all files. If there's a way to keep the quality of the context the same but reduce it's size by removing context or restructuring context, you will do this. Please remove any files which are redundant or unnecessary. Keep the context as simple as possible. 

**Rule 6: System upgrade suggestion**
At the END of every skill run, add one actionable improvement suggestion to `references/system-upgrades.md`. This should be something that the developer (me) isnt doing yet like a workflow gap, missed data opportunity, new automation idea, content repurposing angle or anything else you notice during the run. Be specific, and not generic. This turns every skill into a compointing improvement. 
